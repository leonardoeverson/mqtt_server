...
connectionsDAO.prototype.conn_db_insert = function (dados, callback) {

    conn.beginTransaction(function (err) {
        if (err) {
            callback(err, null);
        }
        conn.query("insert into conn_clients(user_id, client_id, client_address, client_port, device_id, method) values(?, ?, ?, ?, ?, ?)", [dados.user_id, dados.client_id, dados.client_address, dados.client_port, dados.device_id, dados.method], function (err, result) {
            if (err) {
